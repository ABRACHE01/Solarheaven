...
@section('content')
  <div class="container">
    <div class="row">
      <div class="col-md-12">
        <h1>Reviews</h1>
        <a href="{{ route('reviews.create') }}" class="btn btn-success mb-3">Create Review</a>
          ...
          <table class="table table-striped">
            ...
            <tbody>
              @foreach ($reviews as $review)
                <tr>
                  <td>{{ $review->appointment->id }}</td>
                  <td>{{ $review->appointment->client->user->name}}</td>
                  <td>{{ $review->appointment->technician->user->name}}</td>
                  <td>{{ $review->comment }}</td>
                  <td>{{ $review->rating }}</td>
                  <td>
                    <a href="{{ route('reviews.show', ['review' => $review->id]) }}" class="btn btn-primary btn-sm">View</a>
                    <a href="{{ route('reviews.edit', ['review' => $review->id]) }}" class="btn btn-warning btn-sm">Edit</a>
